/**
 * @fileoverview Generates the Netlify _redirects file from site config.
 * This replaces the Eleventy redirects.njk template.
 */
...
let rules = `# Netlify Redirect Rules
# https://www.netlify.com/docs/redirects/

`;

// Site-specific redirects from YAML config
if (site.redirects) {
	for (const redirect of site.redirects) {
		rules += `${redirect.from}                 ${redirect.to} ${redirect.status}\n`;
	}
	rules += "\n";
}

rules += `# External Redirects
...
/chat/help                          https://discord.gg/Qj7tQuW6mB 302!
/chat/blog                          https://discord.gg/GrFKqSDkQ9 302!
/chat/developers                    https://discord.gg/JUhFAjbY6w 302!
/chat/eslint-community              https://discord.gg/B5T3PFZCtC 302!
...
if (site.locals && site.locals.docs_latest) {
	rules += `# Old-Style Docs to New-Style Docs
...
`;
	...
	rules += `# Redirect docs back to English site
/docs/*                             https://eslint.org/docs/:splat 302!
`;
}

// Docs checked into main branch but not released
if (site.locals && site.locals.docs_head) {
	rules += `/docs/head/*                        https://${site.locals.docs_head}/:splat 200!\n`;
} else {
	rules += `/docs/head/*                        https://eslint.org/docs/head/:splat 302!\n`;
}

// Docs for the latest v8.x
if (site.locals && site.locals.docs_v8) {
	rules += `/docs/v8.x/*                        https://${site.locals.docs_v8}/:splat 200!\n`;
} else {
	rules += `/docs/v8.x/*                        https://eslint.org/docs/v8.x/:splat 302!\n`;
}

// Docs for the latest v9.x
if (site.locals && site.locals.docs_v9) {
	rules += `/docs/v9.x/*                        https://${site.locals.docs_v9}/:splat 200!\n`;
} else {
	rules += `/docs/v9.x/*                        https://eslint.org/docs/v9.x/:splat 302!\n`;
}
...
if (site.locals && site.locals.blog === false) {
	rules += `
...
if (site.locals && site.locals.version_support === false) {
	rules += `
# Redirect version support page back to English site
/version-support/                 https://eslint.org/version-support/ 302!
`;
}

// Write to dist/_redirects
fs.mkdirSync(outputDir, { recursive: true });
fs.writeFileSync(path.join(outputDir, "_redirects"), rules.trim());
