...
export default function ThemeSwitcher() {
    ...
    const [isSystemTheme, setIsSystemTheme] = useState(document.documentElement.getAttribute("data-system-theme") === "true");

    const toggleTheme = (newTheme, _isSystemTheme = false) => {
        setTheme(newTheme);
        setIsSystemTheme(_isSystemTheme);
        document.documentElement.setAttribute("data-theme", newTheme);
        document.documentElement.setAttribute("data-system-theme", _isSystemTheme);
        window.localStorage.setItem("theme", newTheme);
        window.localStorage.setItem("system-theme", _isSystemTheme);
    };

    useEffect(() => {
        const media = window.matchMedia("(prefers-color-scheme: dark)");

        function onChangeMedia(event) {
            const newSystemTheme = event.matches ? "dark" : "light";

            if (isSystemTheme) {
                toggleTheme(newSystemTheme, true);
        ...
        <div role="region" className="theme-switcher" aria-labelledby="theme-switcher-label" id="js-theme-switcher">
            ...
            <div className="theme-switcher__buttons">
                <button className="theme-switcher__button js-toggle-button" onClick={() => {
                    toggleTheme("light");
                }} aria-pressed={!isSystemTheme && theme === "light"} data-theme="light">
                ...
                <button className="theme-switcher__button js-toggle-button" id="system-theme-toggle" data-theme="system" onClick={() => {
                    const updatedTheme = window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light";

                    toggleTheme(updatedTheme, true);
                }} aria-pressed={isSystemTheme}>
                    <svg className="theme-switcher__icon" focusable="false" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"></rect><line x1="8" y1="21" x2="16" y2="21"></line><line x1="12" y1="17" x2="12" y2="21"></line></svg>
                    <span>System</span>
                </button>
                <button className="theme-switcher__button js-toggle-button" onClick={() => {
                    toggleTheme("dark");
                }} data-theme="dark" aria-pressed={!isSystemTheme && theme === "dark"} >
