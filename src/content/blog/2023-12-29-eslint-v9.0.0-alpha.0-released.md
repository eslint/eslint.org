...
Flat config is now the default configuration format for ESLint and eslintrc is officially deprecated. beginning in v9.0.0 and eslintrc will be deprecated but will still work. To continue using a eslintrc configuration file, you'll need to set the `ESLINT_USE_FLAT_CONFIG` environment variable to `false`.
