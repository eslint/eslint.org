...
### Node.js < v20.19.x, v21.x, v23.x no longer supported
...
The [`RuleTester#run()`](/docs/next/integrate/nodejs-api#ruletesterrun) method now supports **assertion options**, specifically `requireMessage`, `requireLocation`, and `requireData`, to let developers enforce stricter requirements in rule tests. These options enforce that every invalid test case explicitly checks violation messages, locations, and data, ensuring that a test fails if it doesn't meet the requirements.
